...
from network.encoder_ft import ImageEncoderWithLight
from network.nelfnet import NelfNet
from utils.camera import *
from utils.ray import *


class NelfFt(torch.nn.Module):
    def __init__(
        self,
        ...
        self.register_buffer('celeba_loss', None, persistent=False)
    ...
    def train_loss(self, output):
        ...
        return {
            ...
            'celeba_loss': output['celeba_loss']
        }
    ...
    def render(self, train, model, **data):
        ...
        with torch.set_grad_enabled(train):
            self.encode(
                ...
                data['source_image'], data['source_mask'], data['celeba_image'] if train else None
            )
    ...
    def encode(
        ...
        if celeba_image is None:
            self.source_feature, light_feature = self.image_encoder(self.source_image, False)
        else:
            celeba_mask = celeba_image.max(dim=-1)[0] > 0
            feature, reproduce, light_feature = self.image_encoder(
                torch.cat([self.source_image, celeba_image.unsqueeze(0).permute(0, 3, 1, 2)], axis=0),
                True
            )
            self.celeba_loss = torch.mean(
                torch.abs(reproduce[-1].permute(1, 2, 0) - celeba_image) * celeba_mask.unsqueeze(-1)
            )
            del reproduce

            self.source_feature = feature[:-1, ...]
            light_feature = light_feature[:-1, ...]
